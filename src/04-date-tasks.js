...
function timeSpanToString(startDate, endDate) {
  ...
  const arr = [between / 3600000, between / 60000, between / 1000, between];
  const answer = [];
  for (let i = 0; i < arr.length; i += 1) {
    if (i === 0) {
      answer[i] = Math.trunc(arr[i] % 100).toString().padStart(2, '0');
    } else if (i === 3) {
      answer[i] = Math.trunc(arr[i] % 1000).toString().padStart(3, '0');
    } else {
      answer[i] = Math.trunc(arr[i] % 60).toString().padStart(2, '0');
    }
  }
